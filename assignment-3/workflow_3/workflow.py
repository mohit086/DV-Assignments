...
plt.axis('equal')
plt.savefig('fig17.png', bbox_inches='tight')
...
def categorize_duration(duration):
    if duration <= 5:
        ...
        return 'Long'
def categorize_tenure(duration):
    if duration <= 5:
        return 'Short'
    elif duration <= 15:
        return 'Medium'
        ...
        return 'Long'
def categorize_networth(score):
    max_score = df['NetWorth'].max()
...
source_features = ['LoanDuration', 'JobTenure', 'NetWorth', 'BehaviouralRisk']
all_categories = {}
index = 0
for col in source_features + ['LoanApproved']:
    for value in df[col].unique():
